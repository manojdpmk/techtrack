<?php
	...
	class add {
		...
		public function addclient_reservation($items,$date,$time,$client_id,$assign_room,$timeLimit,$status)
		{
			global $conn;
			...
			// }else{
				if($client_id == 0){
					...
					foreach ($items as $key => $items) {
						$itemsArr = explode("||",$items);
						$sql1 = $conn->prepare('INSERT INTO reservation(reservation_code,member_id,item_id,stock_id,reserve_date,reservation_time,assign_room,time_limit,status) VALUES(?,?,?,?,?,?,?,?,?)');
						$sql1->execute(array($code,$client_id,$itemsArr[0],$itemsArr[1],$date,$time,$assign_room,$timeLimit,$status));
						$count = $sql1->rowCount();
					}
						echo ($count > 0) ? '1' : '0';
				}
	...
	switch ($key){
		...
		$add_function->addclient_reservation($items,$date,$time,$client_id,$assign_room,$timeLimit,$i_status);
		break;
